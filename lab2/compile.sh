...
echo '#include <string>' > /tmp/rg_particles_vertex_shader.cpp
echo 'std::string PARTICLES_VERTEX_SHADER = R"=(' >> /tmp/rg_particles_vertex_shader.cpp
cat src/shaders/particles_vertexshader.glsl >> /tmp/rg_particles_vertex_shader.cpp
echo ')=";' >> /tmp/rg_particles_vertex_shader.cpp

echo '#include <string>' > /tmp/rg_particles_fragment_shader.cpp
echo 'std::string PARTICLES_FRAGMENT_SHADER = R"=(' >> /tmp/rg_particles_fragment_shader.cpp
cat src/shaders/particles_fragmentshader.glsl >> /tmp/rg_particles_fragment_shader.cpp
echo ')=";' >> /tmp/rg_particles_fragment_shader.cpp
...
clang++ -std=c++11 -lglfw -lglew -lassimp -framework OpenGL \
    ...
    /tmp/rg_particles_vertex_shader.cpp \
    /tmp/rg_particles_fragment_shader.cpp \
    src/drawutil.cpp src/shader.cpp src/texture.cpp src/particles.cpp src/mesh.cpp src/main.cpp \
    -o main
